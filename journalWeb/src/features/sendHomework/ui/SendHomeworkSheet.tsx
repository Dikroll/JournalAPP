...
export function SendHomeworkSheet({ homeworkId, homeworkTheme, onClose }: Props) {
  const { file, text, mark, step, error, isLoading, setFile, setText, setMark, submit, reset } =
    useSendHomework(homeworkId, () => setTimeout(() => { reset(); onClose() }, 1500))
      ...
      >
        ...
        <div className="flex items-start justify-between mb-5">
          <div className="flex-1 pr-3">
            <p className="text-base font-semibold text-[#F2F2F2] mb-0.5">Отправить задание</p>
            <p className="text-xs text-[#9CA3AF] line-clamp-2 leading-relaxed">{homeworkTheme}</p>
          </div>
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center py-6 gap-3">
            ...
            <p className="text-sm font-semibold text-[#F2F2F2]">Задание отправлено!</p>
            <p className="text-xs text-[#9CA3AF]">Ожидайте проверки преподавателя</p>
          </div>
        ) : (
