...
export function AppUpdateSheet() {
	...
	return (
		<BottomSheet
			onBackdropClick={isDownloading ? undefined : dismiss}
			zIndex={300}
			maxWidth="max-w-lg"
		>
			<div className="space-y-4">
					...
					<GlassCard>
						...
						<ul className="space-y-1.5">
							{parseChangelogItems(serverInfo.changelog).map((entry, i) => (
								<li
									key={i}
									className="flex items-start gap-2 text-sm text-app-text leading-relaxed"
								>
									{entry.label && (
										<span
											className={`shrink-0 mt-0.5 px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase leading-none ${getChangelogLabelStyle(entry.label)}`}
										>
											{entry.label}
										</span>
									)}
				...
				{!isDownloading && (
					<div className="space-y-2">
						...
						<SheetButton onClick={dismiss}>Напомнить позже</SheetButton>
					</div>
				)}
			</div>
		</BottomSheet>
