...
describe("extractYouTubeId", () => {
	...
	it("youtu.be/ID?si=...", () => {
		expect(extractYouTubeId(`https://youtu.be/${VIDEO_ID}?si=abc123`)).toBe(
			VIDEO_ID,
		);
	});
	...
	it("youtube.com/watch?v=ID", () => {
		expect(
			extractYouTubeId(`https://www.youtube.com/watch?v=${VIDEO_ID}`),
		).toBe(VIDEO_ID);
	});

	it("youtube.com/watch?v=ID&t=30", () => {
		expect(
			extractYouTubeId(`https://www.youtube.com/watch?v=${VIDEO_ID}&t=30`),
		).toBe(VIDEO_ID);
	});
...
describe("getYouTubeThumbnail", () => {
	it("hq (по умолчанию)", () => {
		expect(getYouTubeThumbnail(`https://youtu.be/${VIDEO_ID}`)).toBe(
			`https://img.youtube.com/vi/${VIDEO_ID}/hqdefault.jpg`,
		);
	});

	it("maxres качество", () => {
		expect(getYouTubeThumbnail(`https://youtu.be/${VIDEO_ID}`, "maxres")).toBe(
			`https://img.youtube.com/vi/${VIDEO_ID}/maxresdefault.jpg`,
		);
	});

	it("medium качество", () => {
		expect(getYouTubeThumbnail(`https://youtu.be/${VIDEO_ID}`, "medium")).toBe(
			`https://img.youtube.com/vi/${VIDEO_ID}/mediumdefault.jpg`,
		);
	});
...
describe("toYouTubeEmbed", () => {
	it("возвращает embed URL", () => {
		expect(toYouTubeEmbed(`https://youtu.be/${VIDEO_ID}`)).toBe(
