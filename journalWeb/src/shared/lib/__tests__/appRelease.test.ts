...
describe("isRemoteReleaseNewer", () => {
	...
	it("старее по build → false", () => {
		expect(
			isRemoteReleaseNewer({ ...base, serverBuild: 9, serverVersion: "1.0.0" }),
		).toBe(false);
	});
...
describe("parseChangelogItems", () => {
	...
	it("пустая строка → fallback", () => {
		expect(parseChangelogItems("")).toEqual([
			{ label: null, text: "Описание обновления скоро появится" },
		]);
		expect(parseChangelogItems("   ")).toEqual([
			{ label: null, text: "Описание обновления скоро появится" },
		]);
	...
	it("строки без маркеров остаются как есть", () => {
		expect(parseChangelogItems("Просто текст")).toEqual([
			{ label: null, text: "Просто текст" },
		]);
	});
	...
	it("лейблы регистронезависимы", () => {
		expect(parseChangelogItems("FIX: Баг")).toEqual([
			{ label: "fix", text: "Баг" },
		]);
	});
});
...
describe("toChangelogFeedEntry", () => {
	...
	it("парсит changelog в items", () => {
		expect(toChangelogFeedEntry(release).items).toEqual([
			{ label: null, text: "Фикс" },
		]);
	});
