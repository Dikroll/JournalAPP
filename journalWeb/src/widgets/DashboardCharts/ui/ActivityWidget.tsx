import { Activity, BookOpen, UserCheck, XCircle } from "lucide-react";
		...
		>
			<div className="flex items-center justify-between mb-4 shrink-0">
				<h2 className="text-sm font-bold text-app-text flex items-center gap-2">
					<Activity size={16} className="text-app-muted shrink-0" />
					<span>Активность на неделе</span>
				</h2>
			</div>
			...
			<div className="grid grid-cols-3 gap-2 mb-6 shrink-0">
				<div className="flex flex-col">
					...
					<span className="text-sm font-medium text-[#3B82F6]">
						{data.totalAttended}{" "}
						<span className="text-xs font-normal text-app-muted">занятий</span>
					</span>
				</div>
				<div className="flex flex-col">
					<div className="flex items-center gap-1.5 mb-1">
					...
					<span className="text-sm font-medium">
						<span className="text-[#10B981]">{data.totalGrades}</span>{" "}
						<span className="text-xs font-normal text-app-muted">получено</span>
					</span>
				</div>
				<div className="flex flex-col">
					<div className="flex items-center gap-1.5 mb-1">
			...
			<div ref={ref} className="w-full flex-1 min-h-0 relative -mx-2 px-2">
				{width > 0 && height > 0 && (
					...
					>
						...
						/>
						<Tooltip
							content={<CustomTooltip suffix=" оч." />}
			...
			<p className="mt-2 text-[11px] text-app-muted">
				Очки активности = посещенные пары + полученные оценки за день.
			</p>
		</div>
	);
}
