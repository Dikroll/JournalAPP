...
import { useReviews, type ReviewItem } from "@/entities/review";
	...
	return (
		<div className="grid grid-cols-[200px_1fr] gap-4">
			<div className="flex flex-col gap-1">
				<span className="text-sm font-semibold text-app-text">{review.teacher}</span>
				<span className="text-xs text-app-muted line-clamp-1">{review.specs.join(', ')}</span>
				<span className="text-xs text-app-muted">{formatDate(review.date)}</span>
			</div>
			<div className="flex flex-col gap-2">
				<p className={`text-sm text-app-text ${expanded ? '' : 'line-clamp-2'}`}>
					{review.message}
				</p>
