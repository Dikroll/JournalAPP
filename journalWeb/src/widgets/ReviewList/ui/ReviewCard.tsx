  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div>
          <p className="text-sm font-semibold text-[#F2F2F2]">{review.teacher}</p>
          {review.specs.length > 0 && (
            <p className="text-xs text-[#9CA3AF] mt-0.5">{review.specs.join(", ")}</p>
          )}
        </div>
        <span className="shrink-0 text-xs text-[#6B7280]">{formatDate(review.date)}</span>
      </div>
