		...
		<div className='space-y-4'>
			...
			>
				...
				<div className='grid grid-cols-7 mb-1'>
					{RU_DAYS_SHORT.map(d => (
						<div
							key={d}
							className='text-center text-[10px] text-[#6B7280] font-medium py-1'
						>
							{d}
						</div>
					))}
				...
				<div className='grid grid-cols-7 gap-y-1'>
					...
					{Array.from({ length: daysInMonth }).map((_, i) => {
						...
						return (
							...
							>
								<span
									className={`text-xs font-medium leading-none ${isSelected ? 'text-[#F29F05]' : ''}`}
								>
									{day}
								</span>
								{hasData && (
			...
			{selectedEntries && selectedEntries.length > 0 && (
				<div className='space-y-2'>
					...
					<div
						className='bg-white/5 backdrop-blur-xl rounded-[24px] p-3 border border-white/10'
						style={{ boxShadow: '0 4px 24px 0 rgba(0,0,0,0.3)' }}
					>
						{[...selectedEntries]
