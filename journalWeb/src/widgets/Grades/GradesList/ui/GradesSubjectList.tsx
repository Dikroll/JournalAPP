      ...
      {bySubject.map((subj) => (
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3 mb-4">
            <h3 className="text-sm font-semibold text-[#F2F2F2] leading-snug">{subj.spec_name}</h3>
          ...
          <div className="-mx-4 overflow-x-auto scrollbar-none">
            <div className="flex gap-3 px-4 pb-2 w-max">
              {subj.entries.flatMap((entry) =>
                entry.flatMarks.map(({ type, value }) => (
                  <div
                    key={`${entry.date}-${entry.lesson_number}-${type}`}
                    className="flex flex-col items-center gap-1.5"
                  >
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 ${
                      type === "final"   ? "bg-[#A855F7]/20 border-[#A855F7]" :
                    ...
                    <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium border whitespace-nowrap ${GRADE_TYPE_CONFIG[type].color}`}>
