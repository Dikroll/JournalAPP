...
import { GRADE_TYPE_CONFIG, getGradeColor } from "@/entities/grades/hooks/useGradesGroups"
    ...
    <div className="flex items-center justify-between py-2">
      <div className="flex-1 min-w-0 mr-3">
        <div className="flex items-center gap-2 mb-1">
          {showSubject && (
            <h4 className="text-sm font-semibold text-[#F2F2F2] truncate">{entry.spec_name}</h4>
          )}
          <span className="text-xs text-[#6B7280] flex-shrink-0">Пара {entry.lesson_number}</span>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {entry.attended ? (
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-medium text-[#10B981]">Посетил</span>
            </div>
          ) : (
            <div className="flex items-center gap-1">
              <XCircle size={12} className="text-[#DC2626]" />
              <span className="text-xs font-medium text-[#DC2626]">Пропуск</span>
            </div>
          )}
