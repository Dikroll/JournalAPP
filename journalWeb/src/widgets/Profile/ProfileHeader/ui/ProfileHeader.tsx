...
export const ProfileHeader = memo(
	function ProfileHeader({ user, rank, onRankClick }: Props) {
		...
		const stats = [
			...
			{
				icon: <TrendingUp size={15} className="text-white/90" />,
				label: "Рейтинг",
				value: rank ? `#${rank}` : "—",
				onClick: onRankClick,
			},
				...
				<div className="px-4 pt-4 pb-2">
					...
					>
						...
						<div className="relative grid grid-cols-3 gap-2">
							{stats.map(({ icon, label, value, to, state, onClick }) => {
								const content = (
									<>
	...
	(prev, next) =>
		...
		prev.rank === next.rank &&
		prev.onRankClick === next.onRankClick,
);
