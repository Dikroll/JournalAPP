							...
							<div className="flex items-center justify-between gap-4">
								<div className="flex items-center gap-3">
									<div
										...
										style={{
											background: item.isSpend
												? "var(--color-absent-subtle)"
												: "var(--color-checked-subtle)",
											color: item.isSpend
												? "var(--color-absent)"
												: "var(--color-checked)",
										}}
									...
									<div className="flex flex-col min-w-0">
										<span className="text-sm font-semibold text-app-text truncate">
											{item.isSpend
												? item.title
												: `Начисление ${item.pointTypeLabel.toLowerCase()}`}
										</span>
								...
								<div className="flex items-center gap-6 shrink-0 text-right">
									<div className="flex flex-col">
										<span
											className={`text-sm font-bold ${item.isSpend ? "text-status-absent" : "text-status-checked"}`}
										>
											{item.pointsLabel}
										</span>
