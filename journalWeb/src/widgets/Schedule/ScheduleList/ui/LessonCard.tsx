...
export function LessonCard({ lesson, isCurrent = false, timeLabel, compact = false }: Props) {
		...
		<>
			{compact ? (
				...
				>
					...
					{/* Название */}
					<p className='flex-1 font-semibold text-app-text text-[12px] leading-tight truncate'>
						{lesson.subject}
					</p>
					{/* Время */}
					<div className='flex items-center gap-1 text-app-muted flex-shrink-0'>
						<Clock size={9} />
						<span className='text-[10px]'>{lesson.started_at}–{lesson.finished_at}</span>
					</div>
					{/* Аудитория */}
					...
					{timeLabel && (
						<span className='text-[10px] font-medium text-brand flex-shrink-0'>{timeLabel}</span>
					)}
				</div>
			) : (
				/* ── Полная карточка (мобайл / другой день) ── */
				<div
					className={`rounded-[20px] px-4 py-2 border transition-all flex-1 flex flex-col justify-center ${
						isCurrent
							? 'bg-app-surface-active border-app-border-strong'
							: 'bg-app-surface border-app-border'
					}`}
				...
				>
					...
					<div className='flex items-center gap-1.5 text-app-muted mb-1.5 pl-11'>
						<User size={10} />
