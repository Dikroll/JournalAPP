	...
	return (
		<aside className="w-[14rem] shrink-0 flex flex-col h-full bg-transparent border-r border-app-border select-none">
			...
			<div className="shrink-0 p-4 border-b border-app-border">
				...
				>
					<span className="bg-brand text-white px-1.5 py-0.5 rounded text-[11px] font-extrabold mr-1">
						IT
					</span>
			...
			<div className="shrink-0 p-2 border-t border-app-border space-y-1">
				...
				>
					...
					<div
						className={`relative w-8 h-4 rounded-full transition-colors ${theme === "dark" ? "bg-brand" : "bg-app-border-strong"}`}
					>
						<div
							className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${theme === "dark" ? "translate-x-4" : ""}`}
						/>
					</div>
				</button>
			</div>
