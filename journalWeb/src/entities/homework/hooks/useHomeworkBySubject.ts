...
export function useHomeworkBySubject() {
  ...
  const {
    subjects, knownSpecs,
    setSubjectData, appendSubjectItems, setSubjectExpanded, setSubjectStatus,
  } = useHomeworkStore()
  ...
  const loadSubject = useCallback(
    async (specId: number, specName: string, force = false) => {
      ...
      try {
        const { counters, items } = await homeworkApi.getBySubject(groupId, specId)

        const parsedItems: Record<number, import("../model/types").HomeworkItem[]> = {}
  ...
  const loadMoreForSubject = useCallback(
    async (specId: number, statusKey: number) => {
      ...
      try {
        const currentPage = useHomeworkStore.getState().subjects[specId]?.pages[statusKey] ?? 1
        const nextPage = currentPage + 1

        const newItems = await homeworkApi.getByStatusAndSubject(statusKey, groupId, specId, nextPage)
