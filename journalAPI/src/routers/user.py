...
def _adapt(raw: UpstreamUserInfo) -> UserInfo:
    ...
    return UserInfo(
        student_id=raw.student_id, full_name=raw.full_name, photo_url=raw.photo,
        age=raw.age, gender=raw.gender, birthday=raw.birthday,
        registered_at=raw.registration_date, last_seen_at=raw.last_date_visit,
        study_form=raw.study_form_short_name, level=raw.level, achieves_count=raw.achieves_count,
...
def _adapt_profile(raw: UpstreamUserProfile) -> UserProfile:
    return UserProfile(
        id=raw.id, full_name=raw.ful_name, address=raw.address,
        birthday=raw.date_birth, email=raw.email, photo_url=raw.photo_path,
        is_email_verified=raw.is_email_verified, is_phone_verified=raw.is_phone_verified,
        fill_percentage=raw.fill_percentage,
        phones=[Phone(type=p.phone_type, number=p.phone_number) for p in raw.phones],
        socials=[SocialLink(name=l.name, url=l.value) for l in raw.links if l.value and l.show_link],
        relatives=[
            Relative(
                full_name=r.full_name, relationship=r.relationship, address=r.address,
...
async def get_profile(
    ...
    data = await cache.get_or_fetch(
        key=f"user:profile:{user['username']}",
        ttl=TTL.USER_INFO,
        fetch=lambda: client.get("/profile/operations/settings"),
    )
    return _adapt_profile(UpstreamUserProfile(**data))